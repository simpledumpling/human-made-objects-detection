...
namespace NeuralNetworksLibrary
{
    ...
    public class LayerNW //class Network Layer
    {
        double[,] Weights;
        int cX, cY;
        ...
        protected void GiveMemory() //Allocate memory for weights
        {
            Weights = new double[cX, cY];
        }

        // Конструктор с параметрами. передается количество входных и выходных нейронов
        public LayerNW(int countX, int countY) //LayerNetwork constructor(input neurons, output neurons)
        {
            cX = countX;
            cY = countY;
            GiveMemory();
    ...
    public class NeuralNW //class Network
    {
        LayerNW[] Layers;
        ...
        public NeuralNW(int sizeX, int sizeY) //Set 1-layer Neural Network constructor(number of neurons in input vector, number of neurons in output vector)
        {
            countLayers = 1;
         ...
         */
        public NeuralNW(int sizeX, params int[] layers) //Set n-layer Neural Network constructor(number of neurons in input vector, layer array with number of neurons in current layer)
        {
            countLayers = layers.Length;
            countX = sizeX; //Количество входов
            countY = layers[layers.Length - 1]; //Количество нейронов на последнем слое
            // Размерность выходов нейронов и Дельты
            NETOUT = new double[countLayers + 1][]; //number of output
            NETOUT[0] = new double[sizeX];
            DELTA = new double[countLayers][]; //delta
            ...
            for (int i = 0; i < countLayers; i++) //Цикл по всем слоям
            {
                countY1 = layers[i]; //В этой переменной хранится количество нейронов на текущем слое

                NETOUT[i + 1] = new double[countY1];
                DELTA[i] = new double[countY1];

                this.Layers[i] = new LayerNW(countX1, countY1); //Создаем слой
                this.Layers[i].GenerateWeights(); // Генерируем веса
                countX1 = countY1; // Теперь наши выходы - это входы для следующего слоя
            }
        }

        // Открывает НС
        public NeuralNW(String FileName) // Open Network
        {
            OpenNW(FileName);
        }
        ...
        public void OpenNW(String FileName) // Open Network
        {
            ...
            countLayers = ReadFromArrayInt(binNW, ref k); //Get layer count
            Layers = new LayerNW[countLayers];
        ...
        int GetSizeNW() //Return Network size in bytes
        {
        ...
        public LayerNW Layer(int num) //Return num-th layer of Network
        {
            return Layers[num];
        }

        // Разбивает переменную типа int на байты и записывает в массив
        void WriteInArray(byte[] mas, ref int pos, int value) //Split int on bytes
        {
        ...
        // Разбивает переменную типа double на байты и записывает в массив
